...
namespace Microsoft.Toolkit.Uwp.Helpers
{
	...
	public sealed class SystemInformation
	{
		...
		/// </summary>
		public SystemInformation(IPreferences preferences)
		{
		...
		public TimeSpan AppUptime
		{
			get
			{
				if (LaunchCount > 0)
				{
					...
					var uptimeSoFar = _preferences.Get<long>(nameof(AppUptime), 0);
		...
		public void AddToAppUptime(TimeSpan duration)
		{
			...
			_preferences.Set(nameof(AppUptime), uptimeSoFar + duration.Ticks);
		}
		...
		public void ResetLaunchCount()
		{
			...
			_preferences.Set(nameof(LastResetTime), LastResetTime.ToFileTimeUtc());
			_preferences.Set(nameof(LaunchCount), LaunchCount);
		}
		...
		public void TrackAppUse(IActivatedEventArgs args, XamlRoot? xamlRoot = null)
		{
			...
			{
				LaunchCount = _preferences.Get<long>(nameof(LaunchCount), 0) + 1;
				TotalLaunchCount = _preferences.Get<long>(nameof(TotalLaunchCount), 0) + 1;
				...
				_preferences.Set(nameof(LaunchCount), LaunchCount);
				_preferences.Set(nameof(TotalLaunchCount), TotalLaunchCount);

				LaunchTime = DateTime.UtcNow;

				var lastLaunch = _preferences.Get<long>(nameof(LastLaunchTime), 0);

				LastLaunchTime = lastLaunch != 0
					? DateTime.FromFileTimeUtc(lastLaunch)
					: LaunchTime;

				_preferences.Set(nameof(LastLaunchTime), LaunchTime.ToFileTimeUtc());
				_preferences.Set(nameof(AppUptime), 0L);

				var lastResetTime = _preferences.Get<long>(nameof(LastResetTime), 0);
		...
		private void UpdateVisibility(bool visible)
		{
			...
			else
			{
				...
				_preferences.Set(nameof(AppUptime), uptimeSoFar + subSessionLength);
			}
		}

		private bool DetectIfFirstUse()
		{
			var firstRun = _preferences.Get<bool>(nameof(IsFirstRun), true);
			_preferences.Set(nameof(IsFirstRun), false);
		...
		private (bool IsUpdated, PackageVersion PreviousVersion) DetectIfAppUpdated()
		{
			...
			var version = _preferences.Get<string>(nameof(currentVersion), "");
			if (string.IsNullOrEmpty(version))
			{
				_preferences.Set(nameof(currentVersion), currentVersion);
			}
			else
			{
		...
		{
			var firstUseTime = _preferences.Get<long>(nameof(FirstUseTime), 0);
			if (firstUseTime != 0)
			{
				return DateTime.FromFileTimeUtc(firstUseTime);
			}

			DateTime utcNow = DateTime.UtcNow;

			_preferences.Set(nameof(FirstUseTime), utcNow.ToFileTimeUtc());
		...
		{
			var version = _preferences.Get<string>(nameof(FirstVersionInstalled), "");
			if (!string.IsNullOrEmpty(version))
			{
				return version.ToPackageVersion();
			}

			_preferences.Set(nameof(FirstVersionInstalled), ApplicationVersion.ToFormattedString());
