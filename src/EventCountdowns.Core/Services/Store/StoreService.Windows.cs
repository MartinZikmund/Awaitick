...
{
	private const string EventCountdownsProId = "9PJRM3NWXGBN";
	...
	public async Task<string?> GetPriceAsync()
	{
		...
		var listingsQueryResult = await context.GetStoreProductsAsync([ProductType.Durable.ToString("g")], [EventCountdownsProId]);
	...
	public async Task<bool> HasProAsync()
	{
		if (_hasPro is null)
		{
			...
			_hasPro = result.AddOnLicenses.Any(license => license.Value.SkuStoreId == EventCountdownsProId);
		}
	...
	{
		var context = GetStoreContext();
		var purchaseResult = await context.RequestPurchaseAsync(EventCountdownsProId);
